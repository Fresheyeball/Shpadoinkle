...
* xref:tutorial/immediate-execution.adoc[Skip to part 2: Immediate Execution]
...
We are using `<$>` and a `TupleSection` here to compose our `num` component. This is because `Html Int` and `Html (Int, Int)` obviously do not unify. Because `Html` is merely a Functor.
...
xref:tutorial/immediate-execution.adoc[Go to part 2 ->]
