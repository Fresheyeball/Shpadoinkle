...
Shpadoinkle is a purely functional user interface library, focused on simplicity, performance, and erognomics.
...
<1> The function recieves the current application state (which is just an `Int`)
...
Composable::
There is no need to wire model updates to locations in the dom by elaborate passing of messages and payloads. Components are highly composable. Impediances in types for sub components are resolved with nothing more than `fmap` (typically with some lenses).
