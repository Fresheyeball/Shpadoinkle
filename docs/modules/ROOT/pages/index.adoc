...
.Coalgebras
====
This is modeling the user interface as https://en.wikipedia.org/wiki/F-coalgebra[Coalgebras]. In programming, these ideas are just functions with the following shapes:
...
Where `A` is a concrete type and `F` is a concrete Functor. In this case, we are operating in the `Html'` Functor. This should provide an intuitive example for how user interfaces can be composed.
====
...
That is it. If you can understand the above example, you understand Shpadoinkle, it is that simple. There is no dispatch, no payloads, no event types, no giant update function, no networks of state managers, no templating, no special html syntax, no magic attributes, no digest cycle, and nothing fancy going on.
